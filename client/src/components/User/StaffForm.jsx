    ...
    if (!isEditActive) {
      if (createStaff.isSuccess) {
        if (!createStaff.data?.data?.errors) {
          ...
          triggerTopAlert(
            true,
            createStaff.data?.data?.errors[0].message,
            "danger"
          );
        }
      }
    ...
    if (isEditActive) {
      if (updateStaff.isSuccess) {
        if (!updateStaff.data?.data?.errors) {
          ...
          triggerTopAlert(
            true,
            updateStaff.data?.data?.errors[0].message,
            "danger"
          );
        }
      }
      ...
      <Panel bordered style={{ margin: "10px" }}>
        <Form onSubmit={isEditActive ? () => update() : () => create()}>
          <FormGroup>
            ...
            <Input block onChange={(e) => setUsername(e)} value={username} />
          </FormGroup>
          <FormGroup>
          ...
          <FormGroup>
            ...
            <Input block onChange={(e) => setFirstName(e)} value={firstName} />
          </FormGroup>
          <FormGroup>
            <ControlLabel>Last name</ControlLabel>
            <Input block onChange={(e) => setLastName(e)} value={lastName} />
          </FormGroup>
          <FormGroup>
            <ButtonToolbar>
